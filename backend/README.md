...
> 本次为前端文档重构，将版本号从 V3.3.16 统一为大版本 V3.3，后端 API 与后端文件结构**无变更**。

**前端同步记录：**
- 前端 README.md 全面重构：清理重复内容、按时间线重新组织版本记录、统一版本号为大版本 V3.3
- 根目录 README.md 同步更新版本号
- 后端 README.md 同步更新版本号

详见维护日志 `../Docs/26-07/26-07-04/2026-07-04-readme-refactor-v3.3.md`。
...
**后端新增/变更：**
- 🆕 新增 `_try_wal_recovery()` 函数：WAL 自动回放恢复策略（优先级最高，保留最近提交事务）
- 🔧 重构 `_attempt_db_recovery()`：调整恢复策略优先级（WAL 回放 → `.recover` → `.dump` → 逐表 SELECT → 重建空库）
- 🔧 调整文件操作顺序：**先备份完整三件套 → 再尝试恢复 → 最后删除损坏文件**，避免过早删除 WAL 导致数据丢失
- 📝 新增维护日志：`Docs/26-06/26-06-30/2026-06-30-sqlite-recovery-commands.md`（核心命令记录）

**性能指标：**
| 指标 | 修复前 | 修复后（预期） |
|------|--------|----------------|
| WAL 模式下损坏恢复成功率 | 0%（必丢数据） | >90%（保留最近事务） |
| 无 WAL 文件时恢复成功率 | 依赖 `.dump` 可用性 | 同前（策略 2/3 兜底） |
| 恢复时间 | 秒级（失败即返回） | 秒级（WAL 回放通常 < 1s） |

详见维护日志 `Docs/26-06/26-06-30/2026-06-30-sqlite-recovery-commands.md`。
...
> 本次为前端 Cesium 云层/高级特效模块拆分与维护日志清理，后端 API 与后端文件结构**无变更**。
...
> 本次版本为前端 Cesium 云层/高级特效模块拆分，后端 API 与后端文件结构**无变更**。
