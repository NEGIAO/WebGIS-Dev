...
> **日期**: 2026-06-06 18:58（更新 19:11 - 增加长按拖拽机制）
> **版本**: v3.3.0
> **类型**: Bug 修复 + 移动端适配
...
### 根本原因
1. **OpenLayers 地图拦截触摸事件**：地图 canvas 的 `touchstart` 监听器优先消费事件，用于地图平移手势，导致滑块分割线的触摸事件被拦截
2. **`touchmove` 事件监听器缺少 `{ passive: false }`**：浏览器默认将 `touchmove` 视为 passive listener，无法调用 `preventDefault()` 阻止默认滚动行为
3. **`handleTouchMove` 函数中缺少 `e.preventDefault()` 调用**：即使设置了 `passive: false`，未显式阻止默认行为仍会导致页面滚动
4. **分割线触摸热区过小**：视觉宽度仅 4px，移动端手指触摸精度不足
5. **缺少 `touch-action: none`**：未在 CSS 层面阻止浏览器的默认触摸手势（如平移/缩放）
6. **与地图平移手势冲突**：单指触摸在移动端是地图平移操作，无法同时用于拖拽滑块
...
为解决移动端触摸事件与地图平移手势冲突的问题，实现**长按激活拖拽**机制：

**交互流程**：
1. 触摸分割线 → 启动 300ms 长按计时器
2. 手指保持静止（偏移 < 15px 容差）等待计时器到期
3. 计时器到期 → 激活拖拽模式，触发振动反馈，显示"已激活拖拽"提示
4. 后续手指移动控制滑块位置
5. 手指抬起 → 退出拖拽模式

**防误触设计**：
- 手指在 15px 容差范围内滑动视为长按等待
- 手指移动超过容差范围自动取消长按（判定为地图平移）
- 长按激活后才更新滑块位置，避免意外触发

**视觉反馈**：
- 长按等待中：句柄呼吸发光动画（`@keyframes longPressPulse`）
- 长按激活：句柄高亮放大 + 发光边框
- 激活提示：屏幕中央弹出 "👆 已激活拖拽" 气泡，800ms 后自动消失
- 设备振动：激活时触发 50ms 触觉反馈（需设备支持）
...
*"移动端触摸体验的核心：足够大的热区 + 正确的事件阻止 + 恰到好处的手势控制 + 长按激活避免与地图手势冲突。"*
