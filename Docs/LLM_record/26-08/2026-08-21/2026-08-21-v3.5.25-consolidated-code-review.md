...
本日志为 V3.5.25 统一版本的收敛主记录。终稿范围在上述内容基础上还包括：

1. **原 V3.5.27 增量并入**：公开 `l` 迁移为 1-based 数字（`l=1` 本地瓦片、`l=2` 固定 custom/Wayback）；custom 实际 URL 改由共享运行时 store（`useSharedCustomBasemapUrl.ts`）持有，不再进路由或 prop/event 中转——取代本文「修改内容 §1」中早期的 prop/ref 镜像方案描述，以最终实现为准。
2. **原 V3.5.26 增量并入**：在线信号隔离（鉴权活跃 vs 显式断线心跳）、Wayback 空目录可重试、北京时间展示修正，详见[该日志](2026-08-21-v3.5.26-review-bugfixes.md)。
3. **复核修复批次**：`statistics.py` 在线口径注释更新；启动调度器默认超时对齐 15s；`basemapRegistry.ts` 注释去除旧编号自指；CHANGELOG 合并三条目并移除死链；OL 侧 `l=2` 无共享 URL 时启动回退默认底图（对齐 Cesium 行为）；历史影像读路径建表 DDL 加进程级一次性守卫。
4. **实时统计终稿重设计**（用户指令"统计逻辑与负担全部交后端、准确且实时"）：前端删除心跳定时器/visibilitychange 心跳补发，仅保留 SSE 重连与 rAF 合并；后端新增过期扫描协程（5s 扫描 + 即时广播）、事件驱动合并广播（1s 窗口）、保底周期放宽至 30s、stream 内 `is_disconnected()` 读探活；游客 `guest_uid` 去除 IP 参与（消除网络切换身份分裂）；`/statistics/heartbeat` 端点向后兼容保留。
