...
> ⚠️ **终稿口径以** [V3.5.25 暂存区整合与代码审查](2026-08-21-v3.5.25-consolidated-code-review.md)「收敛补充 §4」为准：本文写作时的"前端 30s 断线心跳兜底"方案已在终稿重设计中移除（前端零主动请求，兜底信号改为鉴权 API 搭车活跃 + 后端过期扫描），广播策略升级为事件驱动。以下为过程记录，供追溯。
