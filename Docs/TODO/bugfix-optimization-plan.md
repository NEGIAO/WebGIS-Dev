...
### P2-5 在线服务 XYZ 刷新后 TOC 组与地图状态不一致（V3.5.37 遗留，需产品决策）
- **问题**：XYZ 经注册表接入后，记录存会话态（刷新即清）+ custom 底图实例退位；但用户自定义 XYZ URL 若走旧持久化通道，
  F5 后地图仍渲染该层而「远程服务」TOC 分组消失，出现"看得见图层、管不了图层"的体验断层。
- **方案候选**：a) XYZ 注册信息持久化到 localStorage 并启动时回灌注册表；b) 明确 XYZ 属临时会话能力，刷新即失效并在 UI 提示。
  倾向 a（与 WMS/WMTS/ArcGIS 行为对齐），但涉及存储格式设计。
- **来源**：日志 [2026-08-25-remote-service-code-review-fixes.md](../LLM_record/26-08/2026-08-25/2026-08-25-remote-service-code-review-fixes.md) 遗留与风险节。
...
- [ ] **KML/KMZ 符号链路顺带发现**（2026-08-18 · V3.5.24 会话，按 Force_command §2.5 只记不改）：
