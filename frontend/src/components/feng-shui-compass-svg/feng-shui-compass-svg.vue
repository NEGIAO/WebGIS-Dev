            ...
            >
                ...
                >
                    <line
                        ...
                        :x2="getScaleEndX(i)"
                        :y2="getScaleEndY(i)"
                        :stroke="i % 10 === 0 ? m.line.scaleHighlightColor : m.line.scaleColor"
                        :stroke-width="borderWidth"
                    />
                    <text
                        v-if="i % 10 === 0"
...
const radius = computed(() => {
    ...
    const availableSpace =
        Math.min(m.value.compassSize.width, m.value.compassSize.height) / 2 - maxScaleSpace;
...
function hasLatticeFill(latticeIndex: number, layerIndex: number) {
    return m.value.latticeFill.some((fill) => fill[0] === latticeIndex && fill[1] === layerIndex);
}
...
const baseFontSize = computed(() => {
    ...
    const userFontSize =
        layerData.fontSize || Math.min(m.value.compassSize.width, m.value.compassSize.height) / 20;
...
function getTogetherTextX(layerIndex: number, textIndex: number, subIndex: number, layer: Layer) {
    return getTogetherTextPosition(layerIndex, textIndex, subIndex, layer).x;
}

function getTogetherTextY(layerIndex: number, textIndex: number, subIndex: number, layer: Layer) {
...
function getLayerBorderPath(layerIndex: number) {
    ...
    if (layer.shape !== 'circle' && layer.shape) {
        ...
        path.push(`M ${centerPoint.value.x + radius} ${centerPoint.value.y}`);
        path.push(
            `A ${radius} ${radius} 0 1 1 ${centerPoint.value.x - radius} ${centerPoint.value.y}`,
        );
        path.push(
            `A ${radius} ${radius} 0 1 1 ${centerPoint.value.x + radius} ${centerPoint.value.y}`,
        );
        ...
        void (Array.isArray(newData) &&
            newData.forEach((layer: any, index: number) => {
                if (layer.shape === 'polygon' && layer.startAngle) {
                    console.warn(
                        `警告: 第${index + 1}层使用了多边形模式，多边形模式不支持startAngle设置。`,
                    );
                    layer.startAngle = 0;
                }
            }));
