      ...
      >
        ...
        <template v-if="layer?.shape === 'circle' || !layer.shape">
          <circle
            ...
            :stroke="getLayerBorderColor(layerIndex)"
            :stroke-width="getLayerStrokeWidth(layerIndex)"
            fill="none"
          />
        </template>
        <template v-else>
        ...
        <path
          ...
          :fill="getLatticeFillColor(latticeIndex, layerIndex)"
        ...
        >
          <text
            ...
            :fill="getLayerTextColor(layer, textIndex)"
            :fill-opacity="getLayerTextOpacity(layer)"
          ...
          <g v-else>
            <text
              ...
              "
              :fill="getLayerTextColor(layer, subIndex)"
              :fill-opacity="getLayerTextOpacity(layer)"
              :font-size="m.autoFontSize ? baseFontSize : layer.fontSize"
              text-anchor="middle"
              dominant-baseline="middle"
...
function getLatticeFillColor(latticeIndex: number, layerIndex: number) {
  ...
  if (fill && fill[2]) return fill[2];
  const layer = m.value.data[layerIndex];
  if (layer?.latticeFillColor) {
    return String(layer.latticeFillColor);
  }
  return "transparent";
}

function getLayerBorderColor(layerIndex: number): string {
  const layer = m.value.data[layerIndex];
  return String(layer?.borderColor || m.value.line.borderColor || "#666");
}

function getLayerStrokeWidth(layerIndex: number): number {
  const layer = m.value.data[layerIndex];
  const widthRaw = Number(layer?.strokeWidth ?? borderWidth);
  if (!Number.isFinite(widthRaw)) return borderWidth;
  return Math.max(0.5, Math.min(8, widthRaw));
}

function getLayerTextColor(layer: Layer, textIndex: number): string {
  if (Array.isArray(layer?.textColor)) {
    return String(layer.textColor[textIndex] || layer.textColor[0] || "#ffffff");
  }
  return String(layer?.textColor || "#ffffff");
}

function getLayerTextOpacity(layer: Layer): number {
  const opacityRaw = Number(layer?.textOpacity ?? 1);
  if (!Number.isFinite(opacityRaw)) return 1;
  return Math.max(0, Math.min(1, opacityRaw));
}
