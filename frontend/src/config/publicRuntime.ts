...
/** 应急游客强制模式（VITE_EMERGENCY_GUEST_MODE，默认关闭）。
 * 后端下线时置 1，前端未登录直接以游客身份进地图浏览；后端恢复后置 0 重建即可一键回原登录流程。
 * 仅应急/临时使用，不属正式功能。 */
export const EMERGENCY_GUEST_MODE: boolean =
    String(import.meta.env.VITE_EMERGENCY_GUEST_MODE || '0').trim() === '1';
...
/** GitHub 开源数据边缘代理（Cloudflare Worker，源码见 NEGIAO.github.io 仓库 workers/github-stats/）基址。
