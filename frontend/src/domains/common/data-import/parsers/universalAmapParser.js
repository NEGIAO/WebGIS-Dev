 ...
 */
import { gcj02ToWgs84 } from '../crs/coordTransform'; // 确保路径正确
...
export function parseAmapDetailAoi(payload) {
    ...
    const aoiNode =
        data?.data?.spec?.mining_shape ||
        data?.spec?.mining_shape ||
        data?.pois?.[0]?.spec?.mining_shape ||
        {};
...
export function parseStandardGeoJson(payload) {
    ...
    data.features.forEach((feature, _index) => {
        ...
        if (!poiid && feature.properties) {
            poiid =
                feature.properties.id || feature.properties.poiid || feature.properties.ID || '';
        }
            ...
            polygonRings.forEach((ring) => {
                if (Array.isArray(ring)) {
                    ...
                    const convertedRing = ring
                        ...
                        .filter(
                            (pt) => pt !== null && Number.isFinite(pt[0]) && Number.isFinite(pt[1]),
                        );
