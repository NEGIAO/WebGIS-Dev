<template>
    <transition name="panel-fade">
        <div
            v-if="selectedPalace && explanationData"
            class="palace-panel-wrapper"
        >
            <!-- 背景装饰 -->
...
import type { SelectedPalace } from '@common/compass/stores/useCompassStore';
import { getThemeExplanationByConfig } from '@common/compass/utils/themeExplanationMapper';
import { ExplanationLookup, type ExplanationResult } from '@common/compass/utils/explanationLookup';
...
const polygonExplanation = computed(() => {
    ...
    return getExplanationByPalace(
        props.selectedPalace,
        themeDataLayers.value,
...
.panel-header {
    ...
    .layer-badge {
        background: linear-gradient(
            135deg,
            rgba($feng-shui-gold, 0.2) 0%,
            rgba($feng-shui-gold, 0.1) 100%
        );
        color: $feng-shui-gold;
        padding: 4px 10px;
        border-radius: 4px;
    ...
    .header-divider {
        ...
        background: linear-gradient(
            90deg,
            rgba($feng-shui-gold, 0.3) 0%,
            transparent 50%,
            rgba($feng-shui-gold, 0.3) 100%
        );
    }
}
