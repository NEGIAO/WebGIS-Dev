...
export const CLOUD_QUALITY_PRESETS = {
  ...
  smooth: {
    ...
    params: {
      ...
      // 云主 raymarch 2/3 分辨率（兼顾帧率与颗粒感），全分辨率合成回主画面；开启体积云时生效
      cloudResolutionScale: 0.67,
      shadowMapSize: 512,
  ...
  balanced: {
    ...
    params: {
      ...
      cloudExposure: 2.8,
      skyToSunRatio: 0.26,
      aerialPerspectiveScale: 0.55,
      atmosphereExposureDay: 0.95,
      atmosphereExposureNight: 0.1,
      atmosphereExposureManual: 0.95,
      atmosphereExposureFollowTimeline: true,
      ...
      // 云主 raymarch 85% 分辨率（明显降低颗粒感，仍低于极致全分辨率成本）
      cloudResolutionScale: 0.85,
      shadowMapSize: 512,
      // BSM 更新节奏：仅节流昂贵 raymarch；CSM 矩阵每帧仍会更新，运动时会强制刷新。
      bsmUpdateInterval: 3,
      shadowResolveEnabled: true,
