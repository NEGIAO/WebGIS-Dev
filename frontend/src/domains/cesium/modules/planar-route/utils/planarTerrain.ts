...
export function createLocalCoordinateFrame(positions: Cesium.Cartesian3[]): LocalCoordinateFrame {
	...
	if (!origin) {
		throw new Error(t('cesium.module.planarRoute.err.localFrameFailed'));
	}
	...
	return {
		...
		toCartographic(point: LocalPoint): Cesium.Cartographic {
			...
			if (!cartographic) {
				throw new Error(t('cesium.module.planarRoute.err.localTransformFailed'));
			}
			return cartographic;
		},
		toCartesian(point: LocalPoint, height: number): Cesium.Cartesian3 {
...
export async function sampleMaximumTerrainHeight(viewer: Cesium.Viewer, frame: LocalCoordinateFrame, polygon: LocalPoint[]): Promise<number> {
	...
	if (!Number.isFinite(maximumHeight)) {
		throw new Error(t('cesium.module.planarRoute.err.invalidAreaTerrainHeight'));
	}
...
export async function sampleMaximumTerrainHeightAlongSegments(viewer: Cesium.Viewer, frame: LocalCoordinateFrame, segments: LocalRouteSegment[]): Promise<number> {
	...
	if (cartographics.length === 0) {
		throw new Error(t('cesium.module.planarRoute.err.missingTerrainSamples'));
	}
	...
	if (!Number.isFinite(maximumHeight)) {
		throw new Error(t('cesium.module.planarRoute.err.invalidRouteTerrainHeight'));
	}
	return maximumHeight;
}
...
export async function createCartesianRouteSegments(
	...
	if (!Number.isFinite(options.absoluteFlightHeight)) {
		throw new Error(t('cesium.module.planarRoute.err.invalidAbsoluteHeight'));
	}
...
async function sampleTerrainHeights(viewer: Cesium.Viewer, positions: Cesium.Cartographic[]): Promise<number[]> {
	...
	if (provider instanceof Cesium.EllipsoidTerrainProvider) {
		throw new Error(t('cesium.module.planarRoute.err.terrainNotLoaded'));
	}
	...
	for (let start = 0; start < positions.length; start += TERRAIN_BATCH_SIZE) {
		...
		for (let index = 0; index < batch.length; index++) {
			...
			if (!Number.isFinite(height)) {
				throw new Error(t('cesium.module.planarRoute.err.terrainSampleInvalid'));
			}
