...
export class RouteRenderer {
	...
	drawFlightPath(segments: CartesianRouteSegment[]): void {
		...
		for (let index = 0; index < segments.length; index++) {
			...
			this.routeEntityIds.push(id);
 			dataSource.entities.add({
 				id,
 				name: segment.type === 'scan' ? t('cesium.module.planarRoute.entity.scanSegment') : t('cesium.module.planarRoute.entity.transitSegment'),
				polyline: {
		...
		if (firstScanPoint) {
			drawStartPoint(firstScanPoint, dataSource);
		}
	...
	clearRoutePreview(): void {
		...
		// 首航点标记随航线预览一并清理（托管数据源内 + 历史残留兜底）
		dataSource?.entities.removeById('air_start_point');
