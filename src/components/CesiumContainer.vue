...
// --- 生命周期 ---
onMounted(async () => {
  // 动态加载 Cesium 资源（仅在组件挂载时加载，不影响首屏）
  if (!window.Cesium && window.loadCesiumResources) {
    console.log('开始加载 Cesium 资源...');
    try {
      await window.loadCesiumResources();
      console.log('Cesium 资源加载完成！');
    } catch (error) {
